...
MATCHES=$OUTPUT'/matches_dim'
RECONSTRUCTION=$OUTPUT/Reconstruction_known
mkdir -p $RECONSTRUCTION

# 1. Compute Structure from Motion
echo '1. Executing Strucuture from Motion'
openMVG_main_ComputeStructureFromKnownPoses -i $OUTPUT/sfm_data.json -m $MATCHES -o $RECONSTRUCTION/cloud_and_poses.bin -d -f $MATCHES/matches.f.bin
