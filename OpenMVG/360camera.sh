...
DATASET='dataset/Image_datasets-main/360_dataset/360_school/images'
...
openMVG_main_SfMInit_ImageListing -i $DATASET -o $OUTPUT -c 7 -g 1 -f 1

#2. Features Computation
echo '2. Executing Features Computation'
openMVG_main_ComputeFeatures -i $OUTPUT/sfm_data.json -o $MATCHES -m AKAZE_MLDB -p HIGH -n 8
...
openMVG_main_SfM -i $OUTPUT/sfm_data.json -m $MATCHES -o $RECONSTRUCTION -s INCREMENTAL -M matches.f.bin -f ADJUST_ALL -c 5

# Spherical Images are not suitable to make a mvs scene
